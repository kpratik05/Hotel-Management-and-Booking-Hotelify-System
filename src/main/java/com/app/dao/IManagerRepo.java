...
public interface IManagerRepo extends JpaRepository<Manager, Integer> {
	...
	@Query("select s from Staff s left join fetch s.department left join fetch s.shift where s.department.deptId=?1")
	public List<Staff> getEmployeesInDepartment(int deptNo);
	
	@Query("select m from Manager m where m.employeeId=?1")
